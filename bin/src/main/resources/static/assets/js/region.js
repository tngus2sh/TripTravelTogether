...
let searchUrl = "../attraction/gungu/";

// 선택된 시도에 따른 구군 값 가져오기
let areaCode, gugunCode;
let mainCity = document.getElementById("search-sido");
...
mainCity.addEventListener("change", function() {
	...
	function makeSubAreaOption(data) {
		...
		for(let i = 1; i < area.length; i++) {
			...
			option.innerText = area[i]["gugunName"];
			option.setAttribute('value', area[i]["gugunCode"]);
			option.setAttribute('class', 'search-content-code');
			option.setAttribute('id',  area[i]["gugunName"]);
